...
void MainWindow::ProcessSegments()
{
    ...
    for (int i = 0; i < n; ++i)
    {
        ...
        {
            //outside
            area -> AddLineSegment(LineSegmentData{QPointF(x1, y1), QPointF(x2, y2), Qt::lightGray});
        }
        else
        {
            qreal A = y2 - y1;
            ...
            while(code1 != 0 || code2 != 0)
            {
                ...
                if (code1 & 1)
                {
                    //leftside xmin
                    qreal y = (-C - A * xmin) / B;
                    area -> AddLineSegment(LineSegmentData{QPointF(x1, y1), QPointF(xmin, y), Qt::lightGray});
                    x1 = xmin;
                    y1 = y;
                }
                else if (code1 & 2)
                {
                    //rightside xmax
                    qreal y = (-C - A * xmax) / B;
                    area -> AddLineSegment(LineSegmentData{QPointF(x1, y1), QPointF(xmax, y), Qt::lightGray});
                    x1 = xmax;
                    y1 = y;
                ...
                else if (code1 & 4)
                {
                    //bottomside ymin
                    qreal x = (-C - B * ymin) / A;
                    area -> AddLineSegment(LineSegmentData{QPointF(x1, y1), QPointF(x, ymin), Qt::lightGray});
                    x1 = x;
                    y1 = ymin;
                }
                else
                {
                    //topside ymax
                    qreal x = (-C - B * ymax) / A;
                    area -> AddLineSegment(LineSegmentData{QPointF(x1, y1), QPointF(x, ymax), Qt::lightGray});
                    x1 = x;
                    y1 = ymax;
                }
                code1 = getCode(x1, y1);
...
void MainWindow::ProcessPoly()
{
    ...
    int x2, y2;
    fin >> x1 >> y1 >> x2 >> y2;
    area -> AddPolygon(points, Qt::lightGray);
